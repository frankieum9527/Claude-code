...
def parse_float_list(s):
    ...
    try:
        ...
        raise RiskInputError(f"expected a comma-separated list of numbers, got {s!r}")
...
def main():
    ...
    if args.mode == "fixed" and args.hard_stop_pct is None:
        raise RiskInputError("--hard-stop-pct is required when --mode fixed")
    if args.mode == "volatility_scaled" and None in (
        ...
        raise RiskInputError(
            "--volatility-multiplier/--min-stop-pct/--max-stop-pct/--fallback-stop-pct "
            "are all required when --mode volatility_scaled"
        )
    if args.take_profit_tier_fired and not args.daily_highs:
        raise RiskInputError("--daily-highs is required when --take-profit-tier-fired is set")
    if args.average_cost <= 0:
        raise RiskInputError(f"--average-cost must be positive, got {args.average_cost}")
    if args.current_price <= 0:
        raise RiskInputError(f"--current-price must be positive, got {args.current_price}")
    if args.mode == "volatility_scaled" and args.min_stop_pct > args.max_stop_pct:
        raise RiskInputError(
            f"--min-stop-pct ({args.min_stop_pct}) exceeds --max-stop-pct "
            f"({args.max_stop_pct}) -- the clamp would invert"
        )
    if any(c <= 0 for c in args.daily_closes):
        raise RiskInputError("--daily-closes must all be positive prices")
    if any(h <= 0 for h in args.daily_highs):
        raise RiskInputError("--daily-highs must all be positive prices")
