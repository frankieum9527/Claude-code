...
def main():
    ...
    if args.starting_capital_usd <= 0:
        raise RiskInputError("starting_capital_usd must be positive")
    if args.daily_limit_pct <= 0 or args.weekly_limit_pct <= 0:
        raise RiskInputError(
            "daily and weekly loss limits must both be positive fractions -- a non-positive "
            "limit would halt entries unconditionally"
        )
