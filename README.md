...
- ✅ Server-synced completions with streaks (`GET/PUT /me/completions`) and a coach adherence card (`GET /teams/:id/adherence` — active days, counts, last active per player). Off-season program exercises are checkable too (by item index), and the streak + adherence count both kinds of work — the training loop survives the season boundary
- ✅ Week-at-a-glance strip on the player's Today (`GET /me/week` — 7 classified days with event and program-session markers); tapping a day previews it. Completing a session pops a celebration banner, with haptic feedback on check-offs (native)
- ✅ Local training reminders (opt-in toggle on Today): a morning heads-up whose copy matches the day (game/practice/home session/off-season session; rest days stay quiet) plus an hour-before nudge for team events — scheduled on-device from the week data via `expo-notifications`, no server push needed. Rescheduled on each app open so schedule changes propagate. Server push (feedback, changes while the app is closed) arrives with the deployment phase
- ✅ UX polish batch: the plan builder pre-fills from the profile (`GET /me/today` returns a `profile` block — age from birthdate is stated, not re-asked; height/weight prefilled); video uploads show real byte-progress (`createUploadTask`); and the Today view keeps an identity-scoped offline cache (`src/todayCache.ts`), so a cold open with no signal shows the last saved day with an "offline" banner instead of an error card
...
- ✅ End-to-end audit (see [`docs/AUDIT.md`](docs/AUDIT.md)): a 91-check adversarial harness (`apps/api/scripts/audit.e2e.mjs`) over a fresh database — cross-team isolation matrix, COPPA consent lifecycle, AI-draft leak checks, guardrail bypass attempts, program completions, week strip, ICS idempotency/health, path traversal. Two passes; fixes so far: consent re-checked at upload time, redirect-validated ICS fetches, birthdate-derived guardrail bands, and a **fail-closed auth guard** (`NODE_ENV=production` refuses to boot in the impersonable `x-user-id` dev-stub mode unless OIDC is configured)
- ⏳ Deployment gate (see the go/no-go checklist in `docs/AUDIT.md`): set `AUTH_ISSUER`, switch Prisma to Postgres, add a cloud storage adapter + rate limiting; then video transcoding and guardian email verification (Phase 3)
